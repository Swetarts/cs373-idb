...
def get_characters():
  ...
  return json.dumps([i.serialize_many_character for i in db.session.query(models.Character).all()])
...
def get_character_detail(id):
  ...
  # results = parsed['results']
  # return json.dumps(results)
  return json.dumps(db.session.query(models.Character).filter_by(id=id).first().serialize)
...
def get_people():
  ...
  # results = parsed['results']
  # return json.dumps(results)
  return json.dumps([i.serialize for i in models.Person.query.all()])
...
def get_person(id):
  ...
  # results = parsed['results']
  # return json.dumps(results)
  return json.dumps(models.Person.query.filter_by(id=id).first().serialize)
