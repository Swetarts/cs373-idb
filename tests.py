...
from models import Character, Person, Comic_Volume, Power, Team, Publisher
...
class FlaskModelTests(TestCase):
 ...
 #    def test_blank_comic_issue(self):
	...
	#     db.session.commit()
	#     c = Comic_Issue.query.filter_by(id='5').first()
	#     assert c.id == 5
	#     assert c.title == None
	#     assert c.image == None
 ...
 #    def test_comic_issue_creation_2(self):
	...
	#     db.session.add(comic)
	#     db.session.commit()
	#     c = Comic_Issue.query.filter_by(id='500').first()
	#     assert c.id == 500
